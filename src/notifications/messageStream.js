...
const slackURL =
  'https://hooks.slack.com/services/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';
...
function createMessageEmitter() {
    ...
    request.post(
      ...
      (error, response, body) => {
        if (!error && response.statusCode === 200) {
          ...
          console.log(
            `[notification-response] - ERROR - code: ${response.statusCode} - `,
            body
          );
        }
      }
  ...
  stream.throttle(5000).onValue(notification => {
    console.log(
      `~~~~~~~ SENDING NOTIFICATION [${notification.level}]: ${notification.msg}`
    );
