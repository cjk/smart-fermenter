...
const durationLimit =
  15000 * 60; /* allow devices to run at most fifteen minutes */
...
const deviceRunningTooLong = (prev, curr) => {
  ...
  const maxDurations = switchHist
    ...
    .filter(
      (v, _k) => v.maxBy(c => c.at).to === 'on'
    ) /* throw away all that wasn't switched on most recently */
    .map((v, _k) =>
      v.maxBy(sw => sw.at).update(e => now - e.at)
    ) /* replace log-info with simple running-duration in ms  */
    .filter(
      d => d > durationLimit
    ) /* go further only if breaking our running-limitation */
    .forEach((d, dev) => {
  ...
  return curr
    .update('rts', rtState =>
      createNotifier(rtState, buildMessage(noticeTemplate))
    )
